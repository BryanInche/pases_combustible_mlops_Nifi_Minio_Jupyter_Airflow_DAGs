...
def preprocess_and_save_delta(ti):
    ...
    #        # Convertir el valor de 'bytes' a una cadena y luego a una lista de enteros
    #        x_str = x.decode('utf-8')  # Decodificar bytes a string
    #        x_list = x_str.strip('{}').split(',')  # Eliminar las llaves y dividir por coma
    #        return len(x_list) if x_list[0] != '' else 0  # Contar los elementos, asegurándose de no contar listas vacías
    #return 0  # En caso de que no sea un tipo esperado

    # Aplicar la función a la columna 'coord_x_pases'
    # datos['numero_pases_carguio'] = datos['coord_x_pases'].apply(contar_pases)

    # Procesar directamente la columna 'coord_x_pases'
    datos['numero_pases_carguio'] = datos['coord_x_pases'].apply(
    lambda x: len(x.decode('utf-8').strip('{}').split(',')) if isinstance(x, bytes) and x.decode('utf-8').strip('{}') != '' else 0
    )
    ...
    spark_df.write.format("delta").mode("overwrite").save(f"s3a://stage3/tabledelta_stage_flujod4m")

# PASO 2 : Definir el DAG (argumentos basicos)
# Definir el DAG
with DAG(
    '12dag_flujod4m_pases_stage',
    description='FLUJO-ETL-D4M-Modelo-Pases',
